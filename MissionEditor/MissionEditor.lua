...
local function createGUI()  
  ...
  if ProductType.getType() == "LOFAC" then
      if locale == 'ru' then 
        Gui.SetBackground('./MissionEditor/images/lofac/loading-window_RU.png')
        Gui.SetWindowText('СПО-НОПП')
      else
        Gui.SetBackground('./MissionEditor/images/lofac/loading-window.png', true)
...
local function addPathTex()
	...
	if __MAC__ then
		ImageSearchPath.pushPath("MAC_Gui/MAC_Data/Images/Splash")
	end
...
function loadPanels(endProgressValue)
	...
	addPanelInfo(setCoordPanel, left_toolbar_w, top_toolbar_h, 450, 100)
...
function onWindowFocused(focused)
	if optionsEditor.getOption("sound.play_audio_while_minimized") == false then
		if focused then
			music.resume()
		else
			music.pause()
...
if guiVariant == "MAC" then
	...
	mmwWeb.create(0, 0, main_w, main_h)
end
