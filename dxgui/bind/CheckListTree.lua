...
function setCheckBoxTree(self, treeHead, nameToCaptionMap)
	...
	if treeHead then
		self.parentCheckBox = self:insertCheckBox(treeHead.name, nil, nameToCaptionMap, 1)							
		if treeHead.childs then
			self:insertCheckBoxTree(treeHead.childs, self.parentCheckBox, nameToCaptionMap, 2)										
		end
	else
...
function getFlags(self)		
	if self.parentCheckBox then
		...
		return self:getCheckBoxFlags_(self.parentCheckBox, flags)
	else
		return nil
	end
end
...
function getNoFlags(self)		
	if self.parentCheckBox then
		...
		return self:getCheckBoxNoFlags_(self.parentCheckBox, flags)
	else
		return nil
	end
end
